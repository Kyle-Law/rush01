...
{
	if (is_row)
		return (is_reverse ? board[fixed][n + 1] : board[fixed][0]);
	return (is_reverse ? board[n + 1][fixed] : board[0][fixed]);
}

char	get_board_value(char **board, int fixed, int i, int is_row)
{
	return (is_row ? board[fixed][i] : board[i][fixed]);
}
...
char	calc_visible(char **board, int fixed, int is_row, int is_reverse, int n)
{
	...
	init_loop_vars(&i, &end, &step, is_reverse, n);
...
int	is_correct_view(char **board, int fixed, int is_row, int is_reverse, int n)
{
	char visible = calc_visible(board, fixed, is_row, is_reverse, n);
	int clue = get_clue(board, fixed, is_row, is_reverse, n);
	return (clue == visible);
}
